...
#    if obj.get_ncols() != 1:
#        data["current axes"].axis_options.append(f"legend columns={obj.get_ncols()}")

    if hasattr(obj, "_ncols") and obj._ncols != 1:  # Use private attribute if available
        data["current axes"].axis_options.append(f"legend columns={obj._ncols}")
    elif hasattr(obj, "ncol") and obj.ncol != 1:  # Use public `ncol` if available
        data["current axes"].axis_options.append(f"legend columns={obj.ncol}")
